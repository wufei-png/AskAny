#!/usr/bin/env python3
"""SSE-based HTTP MCP server for AskAny RAG system.
...
logger = logging.getLogger("askany-mcp-sse")
...
async def handle_list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            name="rag_search",
            description="Search local documents and FAQ using RAG.",
            inputSchema={
                "type": "object",
                "properties": {
...
async def handle_call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        return [TextContent(type="text", text=f"Error executing RAG search: {str(e)}")]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AskAny RAG MCP SSE Server")
    parser.add_argument("--host", default="0.0.0.0", help="Host to bind to")
    parser.add_argument("--port", type=int, default=8001, help="Port to bind to")
    args = parser.parse_args()

    logger.info(f"Starting AskAny RAG MCP SSE server on {args.host}:{args.port}")
