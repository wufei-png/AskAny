...
def test_db_query():
    ...
    if faq_count > 0:
        cursor.execute(
            f"SELECT id, text, embedding IS NOT NULL as has_embedding FROM {settings.faq_vector_table_name} LIMIT 3"
        )
    ...
    if docs_count > 0:
        cursor.execute(
            f"SELECT id, text, embedding IS NOT NULL as has_embedding FROM {settings.docs_vector_table_name} LIMIT 3"
        )
        rows = cursor.fetchall()
        for row in rows:
            print(f"  ID: {row[0]}, Text: {row[1][:50]}..., Has embedding: {row[2]}")
