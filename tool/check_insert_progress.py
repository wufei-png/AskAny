            ...
            if current_rows is not None:
                if prev_rows is not None:
                    ...
                    if rows_diff > 0:
                        ...
                        print(f"Progress: +{rows_diff:,} rows in last {interval}s ({rate:.1f} rows/sec)")
                        if id_diff > 0:
                            print(f"         Max ID increased by {id_diff:,}")
                    else:
                        print("Progress: No new rows detected (insertion may be stuck or completed)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Check insertion progress for docs vector store")
    parser.add_argument(
        "--monitor",
        "-m",
        action="store_true",
