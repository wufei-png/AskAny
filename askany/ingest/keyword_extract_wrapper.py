...
class KeywordExtractorWrapper:
    ...
    def _filter_by_custom_similarity(
        ...
        keyword_embeddings = np.array(
            self.embed_model._get_text_embeddings(keywords)
        )
    ...
    def _filter_duplicate_and_substring_keywords(self, keywords: List[str]) -> List[str]:
    ...
    def extract_keywords(self, query: str) -> Tuple[List[str],List[str]]:
        ...
        print(f"custom matched keywords (similarity >= {settings.custom_keyword_similarity_threshold}): {custom_matched}")
        print(f"Remaining keywords for LLM filtering: {remaining_keywords}")
        # 过滤重复和子串关键词
        remaining_keywords = self._filter_duplicate_and_substring_keywords(remaining_keywords)
        print(f"Filtered remaining keywords (after duplicate and substring removal): {remaining_keywords}")
        # 对剩余关键词进行 LLM 过滤
        if remaining_keywords:
            filtered_keywords = self.llm_extractor.filter_keywords(remaining_keywords, query)
        else:
            filtered_keywords = []
        print("test2")
        ...
        keywords_other = [kw for kw in merged_keywords if kw not in filtered_keywords_set]
        print("test3")
